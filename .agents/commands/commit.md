...
- 2-3문장 이내
- `Co-authored-by`, `Generated-by` 등 AI 에이전트나 도구의 작성 참여를 나타내는 trailer를 자동으로 추가하지 않음
- 에이전트 이름, 도구 이름, 에이전트용 이메일 주소를 제목이나 본문에 넣지 않음
- 사용자가 공동 작성자 표기를 명시적으로 요청한 경우에만 해당 metadata를 추가
...
- 커밋 작성자 정보는 각 개발자의 로컬 Git `user.name`과 `user.email` 설정을 사용함
- 저장소가 공통 작성자 이름·이메일을 강제로 설정하거나 호스트 주소로 추정하지 않음
