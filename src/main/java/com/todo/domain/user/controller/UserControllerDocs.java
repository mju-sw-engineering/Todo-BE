...
public interface UserControllerDocs {
    ...
    @Operation(
            summary = "회원 탈퇴",
            description = """
                    비밀번호 재확인 후 계정을 삭제합니다.

                    개인정보(계정, 프로필, 알림, 읽음 상태, 동의 이력, 이메일 인증 기록)는 실제로 삭제하고,
                    팀 공동 기록은 삭제하지 않고 작성자만 익명화합니다.
                    탈퇴자가 만든 투두와 보낸 채팅, 완료된 참가 기록은 남으며 작성자가 '탈퇴한 사용자'로 표시됩니다.
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "비밀번호 누락",
                    content = @Content(schema = @Schema(hidden = true))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 실패 또는 비밀번호 불일치",
                    content = @Content(schema = @Schema(hidden = true))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "429", description = "비밀번호 확인 시도 초과",
                    content = @Content(schema = @Schema(hidden = true))),
